...
// COMPREHENSIVE CONTENT (3000+ words each)
const blogPosts = {
  'How to Build a Content Strategy That Ranks': [
    ...
    block('Effective content strategy starts with thorough keyword research. You need to find keywords worth targeting before creating any content. Use keyword research tools to discover opportunities, analyze competition levels, and prioritize based on business value.'),
...
async function main() {
  console.log('🚀 Starting content upload with direct HTTP API...\n');

  try {
    // Get blog post document ID
    const post = await querySanity('*[_type == "blogPost" && title == "How to Build a Content Strategy That Ranks"][0]{_id}');
    ...
    console.log('📝 Updating: How to Build a Content Strategy That Ranks');
    console.log('Document ID:', post._id);

    const content = blogPosts['How to Build a Content Strategy That Ranks'];
